    ...
    <View style={styles.table}>
      <View style={styles.tableHeaderRow} fixed minPresenceAhead={20}>
        {columns.map((column, i) => (
          <Text
            key={column.key}
            style={[cellStyle(column, columnWidths[i]), styles.headerCellText]}
          >
            {column.label}
          </Text>
        ))}
      ...
      {totalsRow && (
        <View style={styles.totalsRow} wrap={false}>
          {columns.map((column, i) => (
            <Text
              key={column.key}
              style={[cellStyle(column, columnWidths[i]), styles.bold]}
            >
...
function buildSummarySentence(
  ...
  const parts = [
    `Total ${spec.summaryNoun} ${span}: ${formatCell(totals[primary.key])}`,
  ];
...
function ReportDocument({
  ...
  const groups = spec.groupByDateKey
    ...
    : null;
  const orientation: PageOrientation = chooseOrientation(
    spec.columns,
    spec.rows,
    spec.totals,
  );
                  ...
                  {section.map((group, groupIndex) => (
                    <View key={groupIndex}>
                      ...
                      <ReportTable
                        ...
                        totalsRow={
                          spec.totals
                            ? computeGroupTotals(group.rows, spec.totals)
                            : undefined
                        }
                      />
                    </View>
