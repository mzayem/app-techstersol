...
export type PartnerPayslipPdfData = {
  id: string;
  number: number;
  periodStart: Date;
  periodEnd: Date;
...
function PartnerPayslipDocument({
  payslip,
  qrDataUrl,
  logoDataUrl,
}: {
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.payTo}>
          <View style={styles.payToCol}>
            ...
            <Text style={styles.bold}>{payslip.partner.name}</Text>
            {payslip.partner.phone && <Text>{payslip.partner.phone}</Text>}
            {payslip.partner.email && <Text>{payslip.partner.email}</Text>}
          </View>
