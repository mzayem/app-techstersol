...
import {
  getPartnerLedger,
  summarizePartnerLedger,
} from "@/actions/partners/ledger-queries";
    ...
    <div className="flex flex-1 flex-col gap-6 p-4 sm:p-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Every partner payment, with the project revenue, work cost, and
          expenses behind it — booked automatically the moment a partnered
          project completes.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Tile
          icon={Landmark}
          label="Total revenue"
          value={formatPkr(totals.totalRevenue)}
        />
        <Tile
          icon={Receipt}
          label="Work cost + expenses"
          value={formatPkr(totals.totalWorkCost + totals.totalExpenses)}
        />
        <Tile
          icon={HandCoins}
          label="Total partner share"
          value={formatPkr(totals.totalShare)}
        />
        <Tile
          icon={PiggyBank}
          label="Paid / Pending"
              ...
              <TableRow key={row.id}>
                ...
                <TableCell className="text-right tabular-nums">
                  {row.revenueAmount != null
                    ? formatPkr(row.revenueAmount)
                    : "—"}
                </TableCell>
                <TableCell className="text-right tabular-nums">
                  {row.workCostAmount != null
                    ? formatPkr(row.workCostAmount)
                    : "—"}
                </TableCell>
                <TableCell className="text-right tabular-nums">
                  {row.projectExpensesAmount != null
                    ? formatPkr(row.projectExpensesAmount)
                    : "—"}
                </TableCell>
                ...
                <TableCell>
                  {row.issued ? (
                    <span className="inline-flex items-center rounded-full bg-emerald-500/10 px-2 py-0.5 text-xs font-medium text-emerald-600 dark:text-emerald-400">
                      Paid{" "}
                      {row.payslipNumber
                        ? `(PP-${String(row.payslipNumber).padStart(5, "0")})`
                        : ""}
                    </span>
                  ) : (
