...
import { getPartnerPayslipForVerification } from "@/actions/partners/payslip-queries";
import { VerifyCursor } from "../../[id]/verify-cursor";
...
// Same visual template as the invoice verification page
// (app/verify/[id]/page.tsx) and the team payslip verification page
// (app/verify/payslip/[id]/page.tsx) — same tokens, markup, and fonts —
// adapted for partner payslip fields.
const STYLES = `
  .tsv-root {
    --tsv-bg: #08080a;
    --tsv-surface: #18181c;
...
export default async function VerifyPartnerPayslipPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const payslip = await getPartnerPayslipForVerification(id);
      ...
      <div className="tsv-wrapper">
        <div className="tsv-card">
          <div className="tsv-badge">
            ...
            E-Payslip Verification
          </div>

          <h1 className="tsv-title">
            Digital <span>Payslip</span> Verification
          </h1>

          <p className="tsv-subtitle">
            This page verifies the authenticity of the issued partner payslip.
          </p>
          ...
          {!payslip ? (
              ...
              <div className="tsv-verify-box">
                <div className="tsv-verify-row">
                  ...
                  <div className="tsv-verify-value">
                    No payslip matches this verification link.
                  </div>
                </div>
              </div>
            ...
            <>
              <div className="tsv-verify-box">
                ...
                <Row label="Payslip No." value={formatPartnerPayslipNumber(payslip.number)} />
                <Row label="Paid To" value={payslip.partnerName} />
                <Row
                  label="Period"
                  value={`${formatDate(payslip.periodStart)} – ${formatDate(payslip.periodEnd)}`}
                />
                <Row label="Issue Date" value={formatDate(payslip.issueDate)} />
                {payslip.projectName && <Row label="Project" value={payslip.projectName} />}
                <Row label="Amount" value={`${payslip.amount.toLocaleString()} PKR`} />
              </div>
              ...
              <p className="tsv-footer-note">
                Verification generated by Techstersol. Any modification to the payslip
                invalidates this verification reference.
              </p>
            </>
          )}
